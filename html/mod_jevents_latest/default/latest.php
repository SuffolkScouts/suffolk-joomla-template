...
SCRIPT;
	...
	function displayLatestEvents()
	{
		...
		if (isset($this->eventsByRelDay) && count($this->eventsByRelDay))
		{
			$content .= $this->modparams->get("modlatest_templatetop") || $this->modparams->get("modlatest_templatebottom")? $this->modparams->get("modlatest_templatetop") : '<div class="event-cards row row-cols-1 row-cols-md-3">';
			...
			foreach ($this->eventsByRelDay as $relDay => $daysEvents)
			{
				...
				foreach ($daysEvents as $dayEvent)
				{
					...
					if ($firstTime)
						$eventrow = '<div class="col"><div class="card event-card"><div class="card-body">%s'.'</div></div></div>';
					else
						$eventrow = '<div class="col"><div class="card event-card"><div class="card-body">%s'.'</div></div></div>';
